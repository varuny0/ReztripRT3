...
public class Mobile_BackAndForthNavigation extends BaseTest{
	...
	@FindAll({ @FindBy(xpath = "//*[contains(text(),'Best Price Per Night')]"), })
	
	public WebElement Bestprice;
	
	@FindAll({ @FindBy(xpath = "//*[@class='navbar-table_icon icon -arrow-left']"), })

	public WebElement BackToCalander;

	@FindAll({ @FindBy(xpath = "//input[@name='commit']"), })

	public WebElement Continue;
	
	@FindAll({ @FindBy(xpath = "(//span[@alt='Back Button'])[1]"), })

	public WebElement BackToRoom;
	
	@FindAll({ @FindBy(xpath = "//*[@class='navbar-table_icon icon -arrow-left']"), })

	public WebElement BackToOffer;
	...
	public void gotourl() throws IOException {
		...
		String beurl = ReadProperties.getPropValues("Admin.properties", "MobileURL");
	...
	public void Bestprice() throws InterruptedException {
		...
		String bestpri = SeleniumRepo.driver.findElement(By.xpath("//*[contains(text(),'Best Price Per Night')]/strong")).getText();
	...
	public void Back() {
		...
		if(BackToCalander.isDisplayed()) {
		ExplicitWait(BackToCalander);
		BackToCalander.click();
		//Javascriptexecutor(BackForthNavigate);
		
		System.out.println("Clicked the back button on room listing page");
		Assert.assertTrue(true, "Clicked the back button on room listing page");
		}
		else {
			System.out.println("Unable to Clicked the back button on room listing page");
			Assert.assertFalse(false, "Unable to Clicked the back button on room listing page");
		}		
	}

	public void roomlist() throws InterruptedException {
		Thread.sleep(2000);	
		...
		for(int i=1; i<roomli.size(); i++) {
			...
			System.out.println(roomli.get(i).getText());
		}
	}
	public void room_type(String roomtype) throws InterruptedException {
		...
		System.out.println("clicked on "+roomtype);
	}
