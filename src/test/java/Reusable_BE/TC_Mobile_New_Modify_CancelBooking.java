...
public class TC_Mobile_New_Modify_CancelBooking extends BaseTest
{
	...
	@Test(priority = 2,enabled = true,dataProvider = "MobileBooking",description="Booking Room in Booking Engine",retryAnalyzer=RetryAnalyzer.class)
	public void Flow(HashMap<String, String> h2) throws InterruptedException, IOException {
		...
		try {
			...
			test = extent.createTest("Modify Mobile Booking ");
			logger.info("Modify Mobile Booking");

			mobile.Menu();
			mobile.verify_reservationslink(Email,Confirmationcode);
			mobile.changereservation();

			test = extent.createTest("Cancel Mobile Booking ");
			logger.info("Cancel Mobile Booking");

			mobile.Click_CancelReservation();
			mobile.Popup_title();
